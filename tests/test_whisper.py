...
async def test_transcription_success(whisper_client):
    ...
    with patch("builtins.open", mock_open(read_data="audio data")), patch.object(
        whisper_client.client.audio.transcriptions,
        "create",
        ...
        assert (
            result == expected_transcription
        ), "The transcription result should match the expected output."
...
async def test_transcription_failure(whisper_client):
    ...
    with patch.object(
        ...
        ), "Should handle transcription service failures gracefully."
...
async def test_main_success(caplog):
    ...
    with patch("voicerecorder.VoiceRecorder") as mock_recorder:
        ...
        mock_recorder.return_value.array_to_wav_bytes.return_value = io.BytesIO(
            b"WAV data"
        )
        ...
        with patch.object(
            WhisperSTT,
            "transcribe_audio",
            AsyncMock(return_value="Transcription successful"),
        ):
            await whisper_main()

        assert (
            "Transcription successful" in caplog.text
        ), "Main function should log successful transcription."
...
async def test_main_exception_handling(caplog):
    ...
    with patch(
        "voicerecorder.VoiceRecorder.record_audio_vad",
        side_effect=Exception("Test Error"),
    ):
        await whisper_main()
        assert (
            "An error occurred: Test Error" in caplog.text
        ), "Errors should be logged properly in the main function."
...
async def test_environment_validation_missing_key():
    with patch.dict(
        ...
        with pytest.raises(
            EnvironmentError,
            match="Environment variables for Azure OpenAI Service not set",
        ):
            WhisperSTT()
