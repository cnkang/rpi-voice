...
async def test_record_audio_vad_no_speech(mock_stream):
    mock_stream.return_value.__enter__.return_value.read = AsyncMock(
        return_value=(b"\x00\x00" * 8000, None)
    )
    recorder = VoiceRecorder()
    recorder.sample_rate = 16000
    with patch("webrtcvad.Vad.is_speech", return_value=False) as mock_is_speech:
        mock_is_speech.side_effect = itertools.cycle(
            [False]
        )  # Continuously return False
        result = await recorder.record_audio_vad(
            max_duration=1.0, max_silence_duration=0.5
        )
...
async def test_recorder_timeout(mock_input_stream):
    ...
    with pytest.raises(asyncio.TimeoutError):
        ...
        await asyncio.wait_for(
            voice_recorder.record_audio_vad(max_duration=2.0), timeout=0.1
        )
...
def test_record_audio_with_speech_frames(voice_recorder):
    ...
    is_speech_responses = [True] * 10 + [
        False
    ] * 100  # True for first 10 calls, then False
    ...
    with patch("voicerecorder.sd.RawInputStream", return_value=stream_mock), patch(
        "voicerecorder.webrtcvad.Vad", return_value=vad_mock
    ):
...
def test_main_records_audio_and_logs_duration_and_size(
    mock_time: MagicMock, mock_record_audio_vad: MagicMock
) -> None:
    ...
    with patch("voicerecorder.logging.debug") as mock_debug, patch(
        "voicerecorder.logging.info"
    ) as mock_info, patch("voicerecorder.logging.error") as mock_error:
        asyncio.run(main())
...
def test_not_speech(voice_recorder):
    ...
    voice_recorder._process_audio_frame(
        b"",
        mock_vad,
        recorded_frames,
    ...
    assert (
        current_silence_duration == 6
    ), "Expected current_silence_duration to increment by 1"
...
def test_not_speech(voice_recorder):
    ...
    updated_duration = voice_recorder._process_audio_frame(
        b"",
        mock_vad,
        recorded_frames,
        current_silence_duration,
