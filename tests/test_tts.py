...
def create_tts_instance():
    with patch("tts.TextToSpeech", autospec=True) as mock:
        ...
        instance.get_azure_cognitive_access_token = AsyncMock(
...
async def test_synthesize_speech_valid_response():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        mock_post.return_value = AsyncMock(
            status_code=200, content=b"valid audio content"
        )
...
async def test_synthesize_speech_invalid_content():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        with pytest.raises(RuntimeError):
            await tts_instance.synthesize_speech(
                "This input should return invalid content."
            )
...
async def test_convert_to_ssml_already_formatted():
    ...
    assert (
        result == formatted_ssml
    ), "Should return the same SSML formatted text when already properly formatted."
...
async def test_tts_main(mock_post):
    ...
    with patch(
        "tts.TextToSpeech.synthesize_speech", new_callable=AsyncMock
